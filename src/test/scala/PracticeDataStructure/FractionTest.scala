...
class FractionTest extends AnyFunSuite {
    ...
    test("Add 2/4 to 2/4") {
        assert(frac.add(Fraction(2,4)) compareValue Fraction(1))
    }
    ...
    test("Sub 2/4 from 2/4") {
        assert(frac.subtract(Fraction(2,4)) compareValue Fraction(0))
    }
    
    test("Multiply 2/4 with 0"){
        assert(frac.multiply(0) compareValue 0)
    }
    test("Multiply 2/4 with 2/4"){
        assert(frac.multiply(Fraction(2,4)) compareValue Fraction(1,4))
    }
    test("Multiply 2/4 with 2/2"){
        assert(frac.multiply(Fraction(2,2)) compareValue Fraction(2,4))
    }
    
    test("Divide 2/4 by 1/2"){
        assert(frac.divide(Fraction(1,2)) compareValue 1)
    }
    test("Divide 2/4 by 2"){
        assert(frac.divide(2) compareValue Fraction(2,8))
    }
    ...
    test("FPE-Test: 1/10 + 2/10 = 3/10"){
        assert(Fraction(1,10) add Fraction(1,5) compareValue Fraction(3,10))
    }
    ...
    test("Equal in value overloading"){
        ...
        assert(frac2.compareValue(2) == frac2.compareValue(Fraction(8,4)))
    }
